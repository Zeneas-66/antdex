...
const zhRegion=new Intl.DisplayNames(['zh-CN'],{type:'region'});
const jfetch=async u=>{const c=new AbortController(),tm=setTimeout(()=>c.abort(),15000);try{const r=await fetch(u,{headers:{'User-Agent':'AntDex-data-generator/1.0 (+https://github.com/Zeneas-66/antdex)'},signal:c.signal});if(!r.ok)throw new Error('HTTP '+r.status);return await r.json()}finally{clearTimeout(tm)}};
const norm=s=>String(s||'').toLowerCase().replace(/\([^)]*\)/g,'').replace(/[^a-z0-9]+/g,' ').trim();
async function gbif(r){
 const murl=new URL('https://api.gbif.org/v1/species/match');murl.searchParams.set('name',r.sci);murl.searchParams.set('kingdom','Animalia');const m=await jfetch(murl);
 ...
 const ourl=new URL('https://api.gbif.org/v1/occurrence/search');ourl.searchParams.set('taxon_key',key);ourl.searchParams.set('limit','0');ourl.searchParams.set('facet','country');ourl.searchParams.set('facet_limit','300');const o=await jfetch(ourl);
...
fs.writeFileSync(path.join(root,'data','generated-core-report.json'),JSON.stringify({generatedAt:new Date().toISOString(),source:'GBIF Species API + occurrence country facet',targetCount:targets.length,generated:Object.keys(results).length,failed},null,2)+'\n');
console.log(`target=${targets.length} generated=${Object.keys(results).length} failed=${failed.length}`);for(const x of failed.slice(0,180))console.log('FAIL',x.sci,x.reason);
